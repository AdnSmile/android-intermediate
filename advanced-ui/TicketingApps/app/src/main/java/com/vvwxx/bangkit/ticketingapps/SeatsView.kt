package com.vvwxx.bangkit.ticketingapps
...
class SeatsView : View {
    ...
    private fun drawSeat(canvas: Canvas?, seat: Seat) {
        ...
        backgroundPath.addRect(0F, 0F, 200F, 200F, Path.Direction.CCW) // bikin kotak
        backgroundPath.addCircle(100F, 50F, 75F, Path.Direction.CCW) // kepala di kotak
        canvas?.drawPath(backgroundPath, backgroundPaint) // biar gambarnya keliatan

        // Sandaran Tangan
        val armrestPath = Path()
        armrestPath.addRect(0F, 0F, 50F, 200F, Path.Direction.CCW) // sandara kiri
        canvas?.drawPath(armrestPath, armrestPaint)
        canvas?.translate(150F, 0F) // mengatur jarak antara objek gambar
        armrestPath.addRect(0F, 0F, 50F, 200F, Path.Direction.CCW) // sandara kanan
        canvas?.drawPath(armrestPath, armrestPaint) // biar gambarnya keliatan

        // Bagian Bawah Kursi
        canvas?.translate(-150F, 175F) // ngatur posisi
        val bottomSeatPath = Path()
        bottomSeatPath.addRect(0F, 0F, 200F, 25F, Path.Direction.CCW) // kotak kursi bawah
        canvas?.drawPath(bottomSeatPath, bottomSeatPaint) // biar gambarnya keliatan
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        for (seat in seats) {
            drawSeat(canvas, seat) // menggambar kursi
        }
        ...
        canvas?.drawText(text, (width/2F) - 197F, 100F, titlePaint) // nampilin dan atur posisi tulisan
    }
    ...
    private fun booking(position: Int) {
        ...
        invalidate() // me-refresh metode onDraw dalam SeatsView
    }

    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) { // posisi x y kursi
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        if (event?.action == ACTION_DOWN) {
            // agar kursi bisa ditekan
            when {
                event.x in widthColumnOne && event.y in heightRowOne -> booking(0) // A1
                event.x in widthColumnTwo && event.y in heightRowOne -> booking(1) // A2
                event.x in widthColumnOne && event.y in heightRowTwo -> booking(2) // B1
                event.x in widthColumnTwo && event.y in heightRowTwo -> booking(3) // B2
                event.x in widthColumnOne && event.y in heightRowTree -> booking(4) // C1
                event.x in widthColumnTwo && event.y in heightRowTree -> booking(5) // C2
                event.x in widthColumnOne && event.y in heightRowFour -> booking(6) // D1
                event.x in widthColumnTwo && event.y in heightRowFour -> booking(7) // D2
            }
        }
